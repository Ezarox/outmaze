...
Private two-player rooms use the lightweight WebSocket server on Google Cloud Run. The browser connects to it only after a player opens multiplayer. Cloud Run validates the GitHub Pages origin, holds the temporary room state in memory, and redirects ordinary HTTP visitors back to the GitHub Pages site.
